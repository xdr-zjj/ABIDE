...
def CalPvalue(initial_correlation,permute_correlation):
    permute_correlation = np.array([[permute_correlation[i][j][0] for j in range(len(initial_correlation))] for i in range(len(permute_correlation))])
    initial_correlation = np.array([initial_correlation[i][0] for i in range(len(initial_correlation))])
    pvalue = (permute_correlation > initial_correlation).mean(axis = 0)
    return pvalue
def SplitHalf(x,y,permute = 1000,train_ratio = 0.5):
...
def TrainTestCcaPvalue(x,y,permute = 1000,nums = 10,train_ratio = 0.5):
    pvalues = []
    for i in range(nums):
        pvalue = SplitHalf(x,y,permute = permute,train_ratio = train_ratio)
...
class BuildTree():
    def __init__(self,x,y,z,mtry = 1/3,n_split = 21,nodedepth = 10,nodesize = 40,permute = 1000,nums = 10,train_ratio = 0.5):
        """
    ...
    def GetBestSplit(self,train_x,train_y,train_z):
        num_split_features = max(round(self.mtry*train_z.shape[1]),1)
